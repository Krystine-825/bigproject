...
class ClassController {
  ...
  Future<void> kickMember(String memberId) async {
    final memberDoc = await fireStoreService.getDocument(
      'class_members',
      memberId,
    );
    ...
    await fireStoreService.updateDocument('class_members', memberId, {
      'status': 'kicked',
    });
  ...
  Future<ClassModel> joinClass({
    ...
    final existingMember = memberSnap.docs.any(
      (d) =>
          d.data()['student_id'] == studentId && d.data()['status'] == 'active',
    );
    ...
    String studentName = '';
    try {
      final userDoc = await fireStoreService.getDocument('users', studentId);
  ...
  Stream<List<Map<String, dynamic>>> streamStudentClasses() {
    final studentId = authService.currentUid ?? '';
    return fireStoreService
        .streamWhere('class_members', field: 'student_id', isEqualTo: studentId)
        .asyncMap((snap) async {
            ...
            activeMembers.map((d) async {
              ...
              try {
                ...
                if (_userCache.containsKey(cls.teacherId)) {
                  ...
                  final teacherDoc = await fireStoreService.getDocument(
                    'users',
                    cls.teacherId,
                  );
